...
The familiarization with \glref{ORTE} was done by creating demo application for Android compatible with Shape. Because the port of \glref{ORTE} to Android has been already done in \cite[FEE:vajnar-bc] and is available as library, the main task was compatibility ensurance. The application was designed to be as simple as possible. {\em Publishers} view allows to create new {\em Publisher} of specific color and random shape, {\em Subscribers} view allows to set up {\em Subscribers} of specified colors. Finally, Settings and Help views are present.
